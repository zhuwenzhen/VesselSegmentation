...
#ifdef __APPLE__
int compareIndexedFloat(void *arg, const void *a, const void *b) {
    int *x = (int *)a;
    int *y = (int *)b;
...
float TFunctional6(const Eigen::VectorXf& data) {
    ...
    qsort_r(data_weighted_index.data(), length_r1, sizeof(int),
            data_weighted.data(), compareIndexedFloat);
...
float PFunctional3(const Eigen::VectorXf& data) {
    ...
    fftwf_plan p =
        fftwf_plan_dft_r2c_1d(data.size(), (float *)data.data(), fourier, FFTW_ESTIMATE);
    assert(p != NULL);
    fftwf_execute(p);
